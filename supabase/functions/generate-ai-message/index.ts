...
What questions can I answer for you? 🚀`;
    ...
    // Send it immediately - inlined logic
    console.log('═══ SENDING MESSAGE ═══');
    
    // Get contact data (we need phone/email)
    ...
    if (contactError || !contactData) {
      console.error('Failed to get contact data:', contactError);
    ...
    if (channel === 'sms') {
      console.log('Sending SMS to:', contactData.phone_number);
    ...
    if (agent_id) {
      ...
      if (productAgent) {
        await supabase
          ...
          .eq('id', agent_id);
        console.log(`✅ Updated product_agents metrics`);
      } else {
        ...
        if (agentConv) {
          await supabase
            ...
            .eq('id', agent_id);
          console.log(`✅ Updated agent_conversations metrics`);
        }
      }
    }
    ...
    await supabase
      ...
      .insert({
        ...
        description: `Sent ${originalMessageType} message via ${channel}`,
        metadata: {
